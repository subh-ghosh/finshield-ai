  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center p-8 h-full bg-[#FEF2F2]">
        <AlertTriangle className="h-10 w-10 text-[#E1000F] mb-4" />
        <h3 className="text-[16px] font-bold text-[#E1000F] mb-2">Investigation Failed</h3>
        <p className="text-[13px] text-[#1E1E1E] mb-6 text-center max-w-md">
          {error.message || 'An unexpected error occurred while communicating with the Planner API.'}
        </p>
        <button 
          onClick={onRetry}
          className="bg-[#E1000F] hover:bg-[#c5000d] text-white font-bold py-2 px-6 text-[12px] tracking-wider transition-colors shadow-sm"
        >
          RETRY INVESTIGATION
        </button>
      </div>
  ...
  if (!result) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-center p-8">
        ...
        <button 
          onClick={onRetry}
          className="mt-6 bg-[#E1000F] hover:bg-[#c5000d] text-white font-bold py-2 px-6 text-[12px] tracking-wider transition-colors shadow-sm"
        >
          START INVESTIGATION
        </button>
      </div>
      ...
      <div className="p-6 space-y-6">
        {result.errors && result.errors.length > 0 && (
          <div className="bg-[#FEF2F2] border border-[#FECACA] p-4 flex gap-3">
            <ShieldAlert className="h-5 w-5 text-[#E1000F] shrink-0" />
            <div>
              <h4 className="text-[13px] font-bold text-[#E1000F] mb-1">Warnings Encountered</h4>
              <ul className="list-disc pl-4 text-[12px] text-[#1E1E1E] space-y-1">
