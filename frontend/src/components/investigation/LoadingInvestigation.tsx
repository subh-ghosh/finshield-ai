  ...
  return (
    <div className="h-full flex flex-col items-center justify-center p-8 bg-white">
      <div className="w-16 h-16 relative flex items-center justify-center mb-6">
        ...
        <Activity className="h-6 w-6 text-brand-red animate-pulse" />
      </div>
      
      <h3 className="text-[16px] font-bold text-brand-black mb-2">LangGraph Investigation Running</h3>
        ...
        {[
          ...
          <div key={i} className="flex items-center gap-3">
            <div className={`w-4 h-4 rounded-full flex shrink-0 items-center justify-center ${
              ...
              step.status === 'running' ? 'bg-brand-red animate-pulse' : 'bg-[#E4E7EC]'
            }`}>
              {step.status === 'done' && <div className="w-2 h-2 bg-white rounded-full"></div>}
            </div>
            <span className={`text-[12px] ${
              step.status === 'done' ? 'text-[#10B981] font-medium' :
              step.status === 'running' ? 'text-brand-black font-bold' : 'text-brand-gray'
            }`}>{step.label}</span>
          </div>
        ))}
      </div>
