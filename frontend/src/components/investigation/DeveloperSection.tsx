...
export function DeveloperSection({ result }: { result: PlannerResult }) {
  ...
  const executionTime = result.execution_time_ms ?? result.executionTimeMs ?? 0;
        ...
        <div className="p-4 border-t border-[#E4E7EC] space-y-4 font-mono text-[11px]">
          <div>
            ...
            <span className="text-brand-black break-all bg-white px-2 py-1 border border-[#E4E7EC] rounded-sm">
              {result.correlation_id ?? result.correlationId}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <span className="text-brand-black">{result.planner_status ?? 'COMPLETED'}</span>
            </div>
            <div>
              <span className="text-brand-gray block mb-1">Execution Time</span>
              <span className="text-brand-black">{executionTime.toFixed(2)} ms</span>
            </div>
            <div>
              <span className="text-brand-gray block mb-1">API Calls</span>
              <span className="text-brand-black">{result.api_calls ?? result.apiCalls ?? 0}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {toolCalls.length === 0 && <span className="text-brand-gray">None</span>}
            </div>
          </div>
        </div>
      )}
