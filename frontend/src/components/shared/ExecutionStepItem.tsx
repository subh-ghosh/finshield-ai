...
export function ExecutionStepItem({ step, index }: { step: ExecutionStep; index: number }) {
  ...
  const getIcon = () => {
    switch(step.status) {
      ...
      case 'failed': return <XCircle className="h-3 w-3 text-brand-red" />;
      case 'running': return <Loader2 className="h-3 w-3 text-[#3B82F6] animate-spin" />;
      default: return <Clock className="h-3 w-3 text-brand-gray" />;
    }
  };
  ...
  return (
    <div className="text-[11px] bg-white border border-[#F0F1F3] p-2.5 shadow-sm flex flex-col gap-1.5">
      ...
      {step.error && (
        <div className="mt-1 pl-6 text-brand-red font-mono border-l border-[#FECACA] ml-2 bg-[#FEF2F2] p-1">
          {step.error}
        </div>
      )}
    </div>
